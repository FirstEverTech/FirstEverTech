...
def add_contribution_footer(svg_path: Path, stats_text: str):
    """
    Post-processes an existing contribution-graph.svg (generated by a separate workflow)
    to append a 'Last Month' label and stats bar in the same style as other graphs.
    Safe to call on every run — removes any previously added footer first.
    """
    ...
    # Strip previously injected footer so we don't accumulate duplicates
    content = re.sub(
        r'<!-- stats-footer -->.*?<!-- /stats-footer -->',
        '', content, flags=re.DOTALL
    )

    # Parse SVG viewport width (fall back to viewBox if no width attr)
    svg_w = None
    m = re.search(r'<svg[^>]+width=["\']([0-9.]+)', content)
    if m:
        svg_w = float(m.group(1))
    else:
        m = re.search(r'viewBox=["\'][\d.]+ [\d.]+ ([0-9.]+)', content)
        if m:
            svg_w = float(m.group(1))

    if svg_w is None:
        print("[WARN] contribution SVG: could not determine width, skipping footer",
              file=sys.stderr)
        return
    ...
    footer = (
        f'\n<!-- stats-footer -->\n'
        f'<text x="{cx}" y="-28" text-anchor="middle" dominant-baseline="auto" '
        f'font-family="DejaVu Sans" font-size="11" font-weight="bold" '
        f'fill="{BLUE}">Last Month</text>\n'
        f'<text x="{cx}" y="-10" text-anchor="middle" dominant-baseline="auto" '
    ...
    svg_path.write_text(content, encoding="utf-8")
    print(f"[INFO] contribution footer written to {svg_path}")
